...
<!-- You can start editing here. -->
	...
	?>
			...
			<ol class="comment-list">
				<?php wp_list_comments( array( 'type'=>'comment','before' => '<div class="navigation">' . __( 'Comments : ', '' ), 'after' => '</div>' ) ); ?>
			</ol>
			
			
	<?php 
			paginate_comments_links( array('prev_text' => '&laquo;', 'next_text' => '&raquo;') );
			if(count(get_comments('type=pingback&post_id='.$post->ID) || get_comments('type=trackback&post_id='.$post->ID))){
	?>			<br /><br />
	...
	?>
		...
		?>
				<p>You must be <a href="<?php echo site_url(); ?>/wp-login.php?redirct_to=<?php the_permalink(); ?>">logged in</a> to post a comment.</p>
		<?php 
			}else{ 
